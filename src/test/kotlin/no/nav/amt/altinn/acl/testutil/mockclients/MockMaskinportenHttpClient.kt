package no.nav.amt.altinn.acl.testutil.mockclients

import no.nav.amt.altinn.acl.testutil.MockHttpClient
import no.nav.amt.altinn.acl.testutil.TokenCreator
import org.springframework.http.HttpHeaders
import org.springframework.http.MediaType

class MockMaskinportenHttpClient : MockHttpClient() {
    fun enqueueTokenResponse() {
        val token = TokenCreator.instance().createToken()

        enqueue(
            headers = mapOf(HttpHeaders.CONTENT_TYPE to MediaType.APPLICATION_JSON_VALUE),
            body = """{ "token_type": "Bearer", "access_token": "$token", "expires_in": 3600 }""",
        )
    }
}
